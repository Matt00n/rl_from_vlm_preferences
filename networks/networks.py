...
def identity_reward_postprocessor(reward: jnp.ndarray, 
                                  postprocessor_params: Any):
    del postprocessor_params
    return reward
...
def make_reward_network(
    inp_size: int,
    normalization_fn: Callable = identity_reward_postprocessor,
    hidden_layer_sizes: Sequence[int] = (256, 256),
    activation: ActivationFn = linen.relu) -> FeedForwardNetwork:
    """Creates a policy network."""
    reward_module = MLP(
    ...
    def apply(normalization_params, policy_params, inp):
        rewards = jnp.squeeze(reward_module.apply(policy_params, inp), axis=-1)
        return normalization_fn(rewards, normalization_params)

    dummy_inp = jnp.zeros((1, inp_size))
    return FeedForwardNetwork(
        init=lambda key: reward_module.init(key, dummy_inp), apply=apply)